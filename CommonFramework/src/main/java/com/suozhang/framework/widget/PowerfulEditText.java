...
public class PowerfulEditText extends android.support.v7.widget.AppCompatEditText {
    ...
    public PowerfulEditText(Context context, AttributeSet attrs, int defStyle) {
        ...
        eyeCloseResourseId = ta.getResourceId(R.styleable.PowerfulEditText_eyeClose, R.drawable.icon_biyan);
        eyeOpenResourseId = ta.getResourceId(R.styleable.PowerfulEditText_eyeOpen, R.drawable.icon_kaiyan);
    ...
    private void init() {
        ...
        if (mRightDrawable == null) {
            ...
            if (funcType == TYPE_CAN_CLEAR) {
                //有清除功能，设置默认叉号选择器
                mRightDrawable = getResources().getDrawable(R.drawable.powerful_edit_text_delete_selector);
            ...
            setOnFocusChangeListener(new OnFocusChangeListener() {
                ...
                public void onFocusChange(View v, boolean hasFocus) {
                    if (funcType == TYPE_CAN_CLEAR) {
                        //如果是清除功能
                        String content = getText().toString().trim();
                        if (!TextUtils.isEmpty(content) && hasFocus) {
                            //初始化内容不为空，则不隐藏右侧图标
                            setRightIconVisible(true);
                            setSelection(content.length());
                        } else {
